...
// UI compatibility layer for the current single-page frontend: applies semantic
// budget display (剩餘預算／超支) once the accounting dashboard renders.
// The buildOverviewSheet/buildPettyCashSheet/buildAllocationSheet override versions
// that used to live here (Issue #43) were dead code: this block only runs once
// document.readyState is 'complete', by which point domain.js has already finished
// loading, so those functions don't exist yet in scope here — the
// `typeof buildXxx === 'function'` gates were always false, and accounting-ui.js's
// versions are the ones that actually run (verified at runtime before removing).
